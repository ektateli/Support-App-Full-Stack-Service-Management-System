...
export default function ProjectsPage() {
  ...
  const openEdit = (p) => {
    ...
    setForm({
      ...
      engineer_ids: (p.engineers || []).map((e) => e.id),
    });
    setMsg("");
    setShow(true);
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <button
          onClick={openCreate}
          className="flex cursor-pointer items-center justify-center gap-2 bg-indigo-600 text-white px-5 py-3 rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95 group"
        >
                  ...
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1.5 mt-1">
                            ...
                            <span className="text-[10px] font-mono font-bold text-slate-400 uppercase">{p.project_code}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex -space-x-2 overflow-hidden">
                        {(p.engineers || []).slice(0, 3).map((e, idx) => (
                          <div
                            key={e.id}
                            className=" inline-block h-8 w-8 rounded-lg ring-2 ring-white flex items-center justify-center text-[13px] font-black text-slate-600"
                            title={e.name}
                          >
                            {e.name}
                          </div>
          ...
          >
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 uppercase tracking-wider ml-1">Project Code</label>
                  <div className="relative">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select
                      className="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl outline-none focus:border-indigo-500"
                      value={form.project_type}
                      onChange={(e) => setForm({ ...form, project_type: e.target.value })}
                      required
                    >
                      <option value=""><Type className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />Select Type</option>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      required
                    >
                      <option value=""><Type className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />Select Region</option>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 bg-slate-50 p-4 rounded-[1.5rem] border border-slate-100">
                  {engineers.map(e => (
                    <label
                      key={e.id}
                      className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${form.engineer_ids.includes(e.id)
