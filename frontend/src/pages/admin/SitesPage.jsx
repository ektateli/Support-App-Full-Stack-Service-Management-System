...
export default function SitesPage() {
  ...
  // Simplified state for a single equipment
  ...
  const openEdit = (s) => {
    ...
    // Handle case where equipment might be stored as an array in backend
  ...
  const save = async (e) => {
    ...
    // Convert back to backend structure if it expects an array
    const payload = {
      ...form,
      equipment: [
        {
    ...
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-in max-w-[1600px] mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Site Registry</h2>
          </div>
          <p className="text-slate-500 mt-1 text-sm font-medium">Manage physical locations and hardware installation.</p>
        </div>
        <button
          onClick={openCreate}
          className="flex cursor-pointer items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-3.5 rounded-2xl font-bold shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95 group"
        >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-slate-400 font-medium">Loading sites...</p>
        </div>
      ) : (
        <>
          {/* Desktop Table View */}
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="p-5 font-black text-slate-500 uppercase tracking-widest text-[10px]">Primary Equipment</th>
                    ...
                    <tr key={s.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="p-5">
                        <div className="flex items-center gap-2 text-slate-500 font-bold">
                          ...
                          {s.project_name || "N/A"}
                          {/* {projects.find(p => p.id === s.project_id)?.name || "N/A"} */}
                        </div>
                      </td>
                      <td className="p-5">
                        {eq.name ? (
                        ...
                        ) : <span className="text-slate-300 italic text-xs">No asset assigned</span>}
                      </td>
                  ...
                  {eq.name && (
                    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-medium">Vendor: {eq.vendor || 'Generic'} • Qty: {eq.qty || 1}</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {filtered.length === 0 && (
            <div className="bg-white rounded-[2.5rem] p-16 text-center border border-dashed border-slate-200">
              ...
              <p className="text-slate-400 text-sm mt-1">Adjust filters or create a new site.</p>
            </div>
          )}
        </>
      )}
          ...
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900">
                    {editing ? "Edit Site Setup" : "Register Site"}
                  </h3>
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-[10px] font-black text-indigo-500 uppercase tracking-widest">
                  <Layers className="w-4 h-4" /> Location Context
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-700 ml-1">Site Name</label>
                    <input
                      className="w-full bg-slate-50 border border-slate-200 px-4 py-3.5 rounded-2xl outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 transition-all font-bold text-slate-800 placeholder:text-slate-400 placeholder:font-medium"
                      placeholder="e.g. Mumbai HQ Rack 4"
                      value={form.name}
              ...
              {/* Equipment Details - Simplified Single Section */}
              <div className="space-y-4 pt-4 border-t border-slate-50">
                <div className="flex items-center gap-2 text-[10px] font-black text-indigo-500 uppercase tracking-widest">
                  <HardDrive className="w-4 h-4" /> Technical Asset
                </div>
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
              ...
              <button className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-4 rounded-2xl font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95">
                ...
                {editing ? "Commit Changes" : "Create Site"}
              </button>
            </div>
          </form>
        </div>
