    ...
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-in max-w-[1600px] mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="bg-white p-1 rounded-2xl border border-slate-100 shadow-sm flex items-center self-start sm:self-auto">
          <button onClick={load} className="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-slate-50 rounded-xl transition-all">
      ...
      {loading ? (
        ...
        <>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map(t => (
                  <tr key={t.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="p-5">
                      <span className="font-bold text-slate-800 line-clamp-1 max-w-[280px]">{t.issue}</span>
                    </td>
                    <td className="p-5">
                    ...
                    <td className="p-5 text-right">
                      <Link
                        to={`/tickets/${t.id}`}
                        className="inline-flex items-center justify-center p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm active:scale-90"
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:hidden gap-4">
            {filtered.map(t => (
              <div key={t.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4 hover:border-indigo-200 transition-all">
                ...
                <Link
                  to={`/tickets/${t.id}`}
                  className="flex items-center justify-center gap-2 w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-100"
                >
